"""
stage_e8_qad_overload_diagnostic.py
─────────────────────────────────────────────────────────────
DIAGNOSTIC-INSTRUMENTED COPY of stage_e8_qad.py (v2, fold-fix +
no-rewrap-fix already applied) -- training and quantization logic
below is COMPLETELY UNCHANGED from stage_e8_qad.py. The only
additions are: (1) printing the E8 codebook's coordinate range once,
and (2) after every epoch, ONE aggregated number (all quantized
layers combined) reporting how many post-scaling weight values
(using THIS script's own block-wise scaling: max(|32-block|)/2 --
the value actually fed to the codebook lookup) fall outside that
range.

PURPOSE: stage_e8_qad_normalization.py (paper's row-wise
normalization) reported ~0.7-1.0% of weights outside the codebook
range every epoch. That number is only meaningful relative to a
baseline -- this script measures the SAME quantity (fraction of
pre-quantization values outside the codebook's range) for the
EXISTING (non-normalized) block-wise scaling scheme, so the two
numbers are directly comparable and answer: does row-wise
normalization actually reduce overload compared to what
stage_e8_qad.py already does, or not?

Kept as a SEPARATE file from stage_e8_qad.py (untouched) and from
stage_e8_qad_normalization.py -- per project convention.

Saves to student_model_lattice_e8_diagnostic/ (a distinct dir from
stage_e8_qad.py's student_model_lattice_e8/) so this run doesn't
overwrite the original checkpoint.
...
FIX vs v1 (carried over from stage_e8_qad.py v2):
  ...
  evaluate_model() was actually measuring live. get_quantized_state_dict()
  folds every E8QuantizedLinear's weight into its quantized (on-lattice)
  reconstruction on a CLONED copy of the state_dict -- never in-place
  on the live model. The final-test-evaluation section also does NOT
  re-wrap the reloaded (already-folded) checkpoint, for the same reason.
"""
...
def compute_overload_stats_blockscaled(model, codebook_min, codebook_max):
    """
    DIAGNOSTIC ONLY (added for comparison against
    stage_e8_qad_normalization.py's row-wise overload check --
    training/quantization logic below is completely unchanged from
    stage_e8_qad.py).

    ONE aggregated number (all quantized layers combined): how many
    post-scaling weight values (using THIS script's own block-wise
    scaling scheme -- max(|32-block|)/2, the value actually fed into
    the codebook lookup) fall outside the codebook's fixed coordinate
    range. This is the block-wise-scaling analogue of the row-wise
    check in stage_e8_qad_normalization.py -- same codebook, same
    "value about to be quantized" concept, different scaling scheme --
    so the two numbers are directly comparable and answer: does
    row-wise normalization actually reduce overload versus this
    script's existing per-block scaling, or not?
    """
    total_outside = 0
    total_weights = 0
    for module in model.modules():
        if isinstance(module, E8QuantizedLinear):
            ...
            max_vals = torch.max(torch.abs(W_flat), dim=1, keepdim=True)[0]
            scales = torch.clamp(max_vals / 2.0, min=1e-8)
            W_scaled = W_flat / scales
            outside = ((W_scaled < codebook_min) | (W_scaled > codebook_max)).sum().item()
            total_outside += outside
            total_weights += W_scaled.numel()
...
def main():
    ...
    print(f"\nCodebook coordinate range: [{codebook_min:.4f}, {codebook_max:.4f}]  "
          f"(DIAGNOSTIC -- for comparison against stage_e8_qad_normalization.py's "
          f"row-wise overload check; this is the SAME codebook construction, so "
          f"same range)")
    ...
    for epoch in range(EPOCHS):
        ...
        # DIAGNOSTIC: aggregated overload check using this script's
        # OWN block-wise scaling (max(|32-block|)/2), same codebook
        # range as stage_e8_qad_normalization.py's row-wise check --
        # directly comparable, answers whether row-wise normalization
        # actually reduces overload vs this existing scheme.
        n_out, n_total, pct_out = compute_overload_stats_blockscaled(student, codebook_min, codebook_max)
        print(f"    [DIAGNOSTIC] Block-scaled overload check: {n_out:,} / {n_total:,} "
        ...
        if val_metrics["f1"] > best_val_f1:
            ...
            student.save_pretrained("student_model_lattice_e8_diagnostic", state_dict=quantized_sd)
    ...
    # left to re-derive.
    best_student = AutoModelForSequenceClassification.from_pretrained("student_model_lattice_e8_diagnostic")
