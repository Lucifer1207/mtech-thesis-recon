...
_ap.add_argument("--epochs", type=int, default=30)
_ap.add_argument("--cb", type=float, default=5.0)
_ap.add_argument("--delta0", type=float, default=1.5)
_ap.add_argument("--q", type=int, default=16)
_ap.add_argument("--m", type=int, default=1)
_ap.add_argument("--bits", type=int, default=16)
_ap.add_argument("--batch_size", type=int, default=32)
_args = _ap.parse_args()

_CHUNK_SIZE_CACHE = {}

EXPERIMENT = "B"    # "A" or "B" -- see module docstring
...
if EXPERIMENT == "A":
    ...
    STUDENT_SAVE_DIR    = f"student_model_lattice_e8_normalization_expB_lit_b{_args.bits}_q{_args.q}_m{_args.m}_ep{_args.epochs}"
else:
...
TEMPERATURE       = 2.0
ALPHA_DISTILL     = 0.5
C_B               = _args.cb     # paper's fixed row-safety constant
DELTA0            = _args.delta0
Q                 = _args.q
M                 = _args.m
BITS              = _args.bits
...
def generate_e8_codebook(bits=8):
    N = 2 ** bits
    print(f"Generating E8 lattice points for {bits}-bit codebook layout ({N:,} entries)...")
    candidates = []
    ...
    codebook = [item[1] for item in candidates[:N]]
...
class E8LatticeQuantizer(nn.Module):
    ...
    def forward(self, x):
        CB = self.codebook
        cb_norm = torch.sum(CB ** 2, dim=1, keepdim=True).T
        n = x.shape[0]
        dev_key = str(x.device)
        if x.is_cuda:
            if dev_key not in _CHUNK_SIZE_CACHE:
                free_bytes, _ = torch.cuda.mem_get_info(x.device)
                budget = int(free_bytes * 0.20)
                _CHUNK_SIZE_CACHE[dev_key] = max(1, budget // (2 * max(CB.shape[0], 1) * 4))
            chunk_size = max(1, min(n, _CHUNK_SIZE_CACHE[dev_key]))
        else:
            chunk_size = n
        ...
        while pos < n:
            try:
                ...
                seg = x[pos:end]
                x_norm = torch.sum(seg ** 2, dim=1, keepdim=True)
                distances = x_norm - 2 * torch.matmul(seg, CB.T) + cb_norm
                idx = torch.argmin(distances, dim=1)
                out[pos:end] = CB[idx]
                pos = end
            except RuntimeError as _e:
                if "out of memory" not in str(_e):
                    raise
                torch.cuda.empty_cache()
                cur = max(1, cur // 2)
                if x.is_cuda:
                    _CHUNK_SIZE_CACHE[dev_key] = cur
                print(f"    [OOM caught] halving chunk -> {cur:,}, retrying")
                if cur <= 1:
                    raise
        return out
...
class E8QuantizedLinearNorm(nn.Module):
    def __init__(self, orig_linear, codebook_np, C_b=5.0):
        ...
        self.C_b = C_b
        y_max = DELTA0 * (Q ** M - 1) / 2.0  # literal paper formula, not empirical
...
def main():
    ...
    codebook_np = generate_e8_codebook(BITS)
