...
int main() {
    ...
    std::cout<<"══════  TinyBERT-QAD E8 8-bit ══════\n";
    { std::vector<block_e8_8> m;
      if(!stream_load("/data/local/tmp/tinybert_e8_qad_8bit.bin", m, "TinyBERT E8-8bit")) return 1;
      rss_load=get_rss_kb();
      auto fp=bench_fullpass(m,x,[](const block_e8_8&b,const float*x){return dot_e8_8(b,x);},5);
      // Change these values to 312 for TinyBERT
      double lu = bench_layer(m, [](const block_e8_8& b, const float* x) { return dot_e8_8(b, x); }, 312, 312, lcs); // Changed from 768 to 312
      print_result("TinyBERT E8-8bit (ReCon)", rss_load, fp.rss, fp.ms, lu, fp.cpu, fp.checksum, lcs, 312, 312, 2.0, 3.87);
    }
